      ...
      private
        ...
        def write_attributes(values)
          unless self.class.class_attributes.nil?
            attr_values = values.slice(*self.class.class_attributes.keys)
            attr_values.each{|k,v| self.class.class_attributes[k] = v}
          end
        end
        
        # Returns a hash without the attribute pairs from the values hash.
        def remove_attributes(values)
          values.except(*self.class.class_attributes.keys) rescue values
        end
        
        # Returns a new hash with all the keys (except xmlns)
        # camel cased.
        def camelized_attributes(attributes = self.class.class_attributes)
          attrs = {}
