...
describe Header do
  ...
  before(:each) do
    @header = Header.new(:source_id => "hello")  
  end
  ...
  it "should have a GUID/UUID that matches SIF textual format" do
    @header.msg_id.value.should_not match(/^[a-z\-\s]+$/)
    @header.msg_id.value.should match(/^[A-F0-9]{32}$/)
  end
...
#    xml = SystemControl.cancel_requests("test cancel", NOTIFICATION_TYPES[:none], "1"*32,"2"*32,"3"*32)
#    body = Message.acdc(xml.acdc)
...
#    body = Message.acdc(xml)
#    body.should be_instance_of(Ack)
#    body.status.code.should eql("0")
#    body.status.data.should be_instance_of(Request)
#   end
# 
...
#    body = Message.acdc(xml)
#    body.should be_instance_of(Request)
#   end
#      
end
# 
